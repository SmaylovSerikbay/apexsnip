...
const (
	...
	// Time allowed to wait for the next frame (data or control). Increased + refreshed per read (see receiveMessages).
...
func (c *Client) receiveMessages() {
	for {
		...
		case <-c.connCtx.Done():
			return
		default:
			// PATCH(apexsnip): extend deadline before each read so high-volume subscription
			// traffic keeps the connection alive (upstream only extended deadline on RFC6455 Pong).
			c.conn.SetReadDeadline(time.Now().Add(pongWait))
